...
#     print(f"L2 Loss, input_shape: {K.shape(inp)}, out_VAE's shape: {K.shape(out_VAE)}")
    loss_L2 = K.mean(K.square(inp - out_VAE), axis=(1, 2, 3, 4)) # original axis value is (1,2,3,4).
...
def loss_VAE(input_shape, inp, out_VAE, z_mean, z_var, weight_L2=0.1, weight_KL=0.1):
...
def build_model(input_shape=(4, 160, 192, 128), output_channels=3, weight_L2=0.1, weight_KL=0.1, n_gpu=None, test_mode=False):
    """
    ...
    c1=c//4 if test_mode == False else c
    x = Dense(c1 * (H//16) * (W//16) * (D//16))(x)
    x = Activation('relu')(x)
    x = Reshape((c1, (H//16), (W//16), (D//16)))(x)
    x = Conv3D(
        filters=256,
        kernel_size=(1, 1, 1),
        strides=1,
    ...
    ### 191214, for testing whether the layers is trained which are not included by the compiled output route .
    out_VAE_layer = Conv3D(
        filters=c,
        kernel_size=(1, 1, 1),
        strides=1,
        kernel_regularizer = l2(l2_reg_weight),
    ...
    ### Output Block
    out_VAE = out_VAE_layer(x)

    # Build and Compile the model
    out = out_GT
    template_model = Model(inp, outputs=[out, out_VAE])  # Create the model
    if n_gpu >= 2:
    ...
    model.compile(
        adam(lr=1e-4),
        [loss_gt(), loss_VAE(input_shape, inp, out_VAE, z_mean, z_var, weight_L2=weight_L2, weight_KL=weight_KL)],
        metrics=[dice_coefficient]
    )

    if n_gpu >= 2:
        return model, template_model, out_VAE_layer, out_GT_layer
    else:
        return model
